export class inputRadio{
  ...
  public readonly type:string = "inputRadio";
  ...
  constructor(public readonly label : string,
    ...
    public static create(label: string, description: string, options: Array<string>=[]) : inputRadio{
      if(options.length>0){
        ...
        if(options.every(x=>{
          ...
          return new inputRadio(label,description,options);
        } else {
          throw new Error("Two or more options share the same label");
        };
      } else {
        return new inputRadio(label,description);
      };
    };
